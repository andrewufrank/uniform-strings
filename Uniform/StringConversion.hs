...
prop_latinT2B :: Text ->   Bool  -- fails rarely...
prop_latinT2B t = if all  ((<256) . ord)(t2s t) then  (inverts Encoding.decodeLatin1 encodeUtf8 t) else True
-- in this case the encodeUtf8 does work

prop_latinB2T :: ByteString ->   Bool  -- fails with encodeUtf8
prop_latinB2T  = inverts encodeLatin' Encoding.decodeLatin1


decodeLatin' :: ByteString -> Text
decodeLatin' = T.pack .  Data.ByteString.Char8.unpack
encodeLatin' :: Text -> ByteString
encodeLatin' = Data.ByteString.Char8.pack . T.unpack

prop_latinT22B :: Text -> Bool
prop_latinT22B t = if all  ((<256) . ord)(t2s t) then  (inverts decodeLatin' encodeLatin' t) else True

test_latin :: IO ()
test_latin = do
    putIOwords ["latin start"]
    conv <- open "ISO-8859-1" Nothing
    let lat = ByteString.pack [198, 216, 197, 206, 219,140,252,202]
--    let uni = "ÆØÅ\DEL\200\SOH\206\219\140\252\202" :: Text
    let a =   toUnicode conv lat :: Text
    let b =   fromUnicode conv a
--    let c = fromUnicode conv uni
--    let d = toUnicode conv c :: Text
--    putIOwords [ uni]
    putIOwords [a ]
...
test_latin2 :: IO ()
-- check decodeLatin (bytestring to text) against icu conv
test_latin2 = do
    conv <- open "ISO-8859-1" Nothing
    let lat = ByteString.pack [198, 216, 197, 206, 219,140,252,202]
    let lat1 = toUnicode conv lat
    let lat2 = Encoding.decodeLatin1 lat
    putIOwords ["latin2 test", lat1, lat2 ]
    assertEqual lat1 lat2

test_latin3 :: IO ()
-- check encode latin (text to bytestring) against icu conv  fails
test_latin3 = do
    conv <- open "ISO-8859-1" Nothing
    let  uni = "ÆØÅ\DEL\200\SOH\206\219\140\252\202" :: Text
    let uni1 = fromUnicode conv uni
    let uni2  = encodeLatin' uni --  encodeUtf8 uni
    putIOwords ["latin3 test", uni]
    assertEqual  uni1 uni2
--    ByteString.putStrLn   uni1
--    ByteString.putStrLn   uni2
    putIOwords ["latin3 test end"]

test_latin4x :: IO ()
-- check decodeLatin (bytestring to text) against icu conv and back
test_latin4x = do
    putIOwords ["latin4 test start" ]
    conv <- open "ISO-8859-1" Nothing
    let lat = ByteString.pack [198, 216, 197, 206, 219,140,252,202] :: ByteString
    let lat1 = toUnicode conv lat
    let lat2 = Encoding.decodeLatin1 lat
    putIOwords ["latin4 test 1", lat1, lat2 ]
    assertEqual lat1 lat2

test_latin5 :: IO ()
-- check decodeLatin (bytestring to text) against icu conv and back
test_latin5 = do
    putIOwords ["latin5 test start" ]
    conv <- open "ISO-8859-1" Nothing
    let lat = ByteString.pack [198, 216, 197, 206, 219,140,252,202] :: ByteString
    let lat1 = toUnicode conv lat
    let lat2 = Encoding.decodeLatin1 lat
    putIOwords ["latin5 test 1", lat1, lat2 ]
--    assertEqual lat1 lat2
    let lat3 = fromUnicode conv lat1 :: ByteString
    let lat4 = encodeLatin' lat1 -- encodeUtf8 lat1 :: ByteString
    assertEqual lat3 lat  -- is ock
    putIOwords ["latin5 test 2" ]

test_latin6 :: IO ()
-- check decodeLatin (bytestring to text) against icu conv and back
test_latin6 = do
    putIOwords ["latin6 test start" ]
    conv <- open "ISO-8859-1" Nothing
    let lat = ByteString.pack [198, 216, 197, 206, 219,140,252,202] :: ByteString
    let lat1 = toUnicode conv lat
    let lat2 = Encoding.decodeLatin1 lat
    putIOwords ["latin6 test 1", lat1, lat2 ]
--    assertEqual lat1 lat2
    let lat3 = fromUnicode conv lat1 :: ByteString
    let lat4 = encodeLatin' lat1 -- encodeUtf8 lat1 :: ByteString
    assertEqual lat3 lat4  -- is ock
    putIOwords ["latin6 test 2" ]


-}
{-
testUrlEncoding :: String -> Bool
testUrlEncoding t = isJust . SN.urlDecode . s2b $ t
...
test_testUrlEncoding = assertEqual True (testUrlEncoding "\65534")
test_test_65534 = assertEqual "\65534" (u2t . t2u $ "\65534")
...
--
--    let b2 = s2b s1
--        a2  = fromJustNote "space to bytestring" . b2u $ b2
